...
# Leggi il CSV
df = pd.read_csv('evaluation/memory/results_rog.csv')
...
plt.plot(df['file_name'], df['mem_usage_mb'], color='lightblue', linewidth=4, marker='o', markersize=12)
...
plt.fill_between(df['file_name'], df['mem_usage_mb'], color='lightblue', alpha=0.2)
plt.tight_layout()
